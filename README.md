...
| **Magic grab** | Canva-style object lifting: drag around a seal, signature or any object to cut it off the page — the background is keyed out, the original spot is patched with the sampled page color, and the cutout becomes a movable, resizable object. |
| **AI workbench** | Bring-your-own open-source AI (Ollama/LM Studio locally for unlimited free use, or any OpenAI-compatible endpoint): generate a styled, downloadable PDF from any topic (with a multi-pass **deep research** mode), and **fill forms with AI** from unstructured text. The endpoint/key live only in your browser. |
