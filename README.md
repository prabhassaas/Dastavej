...
| **AI workbench** | Bring-your-own open-source AI — no server, no API key required from us. Point it at a local model (**Ollama**, **LM Studio**) for unlimited, fully private use, or any OpenAI-compatible provider (Groq, OpenRouter…). Generate a styled, presentation-quality PDF from any topic (with a two-pass "deep research" mode) or fill a form from unstructured text. Provider settings collapse to a one-line summary once configured, and can be exported/imported as a backup file since the key only lives in this browser's storage. For local models, start the server with CORS open to the browser, e.g. `OLLAMA_ORIGINS=* ollama serve` — the panel explains this and gives an actionable error if it's missed. |
| **AI assistant (floating)** | A chat bubble in the bottom-right corner of every page, sharing the same AI configuration as the workbench above. Chat freely, generate content and download it straight to PDF, or ask "how do I split a PDF" / "how do I add a watermark" for step-by-step feature guidance. Ships defaulting to a local **Ollama** instance (`localhost:11434`) out of the box — no key needed, since "localhost" only ever resolves to each visitor's own machine. |
| **RAG — chat with your document** | In the AI tab: the open document's text is chunked and embedded (via the configured provider's embedding model, e.g. Ollama's `nomic-embed-text`), then searched locally with cosine similarity. Questions are answered only from the retrieved excerpts — grounded in what the document actually says, with page sources shown, instead of guessing from general knowledge. |
| **RAG — ask about the API** | On `/developers`: the same retrieval pipeline over the public API's real function reference and code examples, so questions about `lib/publicApi.ts` quote actual signatures instead of a paraphrase, and stay accurate as functions are added. |
...
| **Converter** | PDF → **Word** (`docx`, text with page breaks and heading sizes — scanned pages with no text layer are OCR'd automatically so they still convert), **Excel** and **CSV** (SheetJS — text-position clustering infers real table columns and keeps blank cells blank instead of shifting values left, all pages in one sheet), **PowerPoint** (`pptxgenjs`, each page as a full-bleed slide image) and **Images** (one PNG/JPEG per page, zipped) — all generated in the browser. |
